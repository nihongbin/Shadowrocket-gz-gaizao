...
- `scripts/merge-s0-private-config.py`：把用户本地原始完整配置和 S0/S1/S2 模板合并，输出到 `local/private-configs/` 的私有完整配置。
...
- S0/S1/S2 仍是场景验证模板，不是最终配置。
- S0/S1/S2 公开模板不能单独代表手机最终测试配置；私有合并版因含节点信息不得进入仓库。
- S1 引入远程 `RULE-SET` 依赖，若手机端无法加载远程规则，不能直接判定底层 DNS 方案失败，应回退 S0 再判断。
- S2 更安全但可能牺牲部分中国 App 速度；如果中国 App 日志显示走代理，优先补种子表。
- 本地 `.git` 目录存在但 `git status` 不可用；本轮不修复、不提交、不推送。
- 远端 GitHub 仓库正在按 S1.1 计划重新创建；推送后 GitHub Actions 和 Pages 才能在线运行。
